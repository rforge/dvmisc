...
logodds.graph <- function(y, group, error.bars = "none", alpha = 0.05,
  ...
  if (error.bars == "exact.ci") {
    ...
    probs.ci <- apply(y.group.table, 1, function(x) binom.test(x = rev(x))$conf.int)
  ...
  # Plot and return log-odds for y across x groups
  plot(log.ors, ...)
  return(list(x = x, y = y, logodds = logodds))
