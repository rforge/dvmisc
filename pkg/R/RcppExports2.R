...
y, group, error.bars = "t.ci", alpha = 0.05, 
plot.list = NULL, 
lines.list = NULL, 
axis.list = NULL)
logodds.graph(y = y, group = group, error.bars = "exact.ci")
...
logodds.graph <- function(y, group, error.bars = "none", alpha = 0.05,
  ...
  probs <- tapply(y, group, mean)
  logodds <- log(probs / (1 - probs))
  
  # Create error bars
  if (error.bars == "exact.ci") {
    
    y.group.table <- table(group, y)
    probs.ci <- apply(y.group.table, 1, function(x) binom.test(x = rev(x), conf.level = 1 - alpha)$conf.int)
    ...
    y.group.table <- table(group, y)
    probs.ci <- apply(y.group.table, 1, function(x) prop.test(x = x[2], n = sum(x), conf.level = 1 - alpha)$conf.int)
    logodds.ci <- log(probs.ci / (1 - probs.ci))
    lower.bars <- logodds.ci[1, ]
    upper.bars <- logodds.ci[2, ]
    y.label <- paste(y.varname, " (log-odds +/- 95% CI)", sep = "")
...
x <- rnorm(10000)
y <- rbinom(10000, size = 1, prob = (1 + exp(0 - 1 * x))^(-1))
log.or(x = x, y = y, pch = 16, col = "red")
